...
'--------------------------------------------------------------------
' author_delete.asp
'	Delete an existing article author from the database
'
...
If steForm("action") = "delete" Then
	...
	Else
		...
		sStat = "DELETE FROM tblArticleAuthor " &_
...
If nAuthorID > 0 Then
	sStat = "SELECT * FROM tblArticleAuthor " &_
			"WHERE AuthorID = " & nAuthorID
...
<H3><% steTxt "Delete Article Author" %></H3>

<P>
<% steTxt "Please confirm the deletion of the article author by clicking the yes button next to <I>Confirm</I>." %>
</P>
...
<H3><% steTxt "Article Author Deleted" %></H3>

<P>
<% steTxt "The article author was successfully deleted from the database." %>&nbsp;
