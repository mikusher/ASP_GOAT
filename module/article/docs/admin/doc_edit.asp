...
If UCase(steForm("action")) = "EDIT" Then
	...
	Else
		nOrderNo = steNForm("OrderNo")
		If nOrderNo <= 0 Then nOrderNo = 1
		...
		sStat = "UPDATE tblDoc SET " &_
				"	BookID = " & steNForm("BookID") & "," &_
				"	ParentDocID = " & steNForm("ParentDocID") & "," &_
				...
				"	SectionName = " & steQForm("SectionName") & "," &_
				"	SectionNo = '" & sSectionNo & "', " &_
				"	IsInlineContent = " & steNForm("IsInlineContent") & " " &_
				"WHERE DocID = " & nDocID
...
<H3><% steTxt "Edit Document" %></H3>

<P>
<% steTxt "Please enter the changes to the document in the form below." %>&nbsp;
<% steTxt "When you are finished, click the Update Document button to finalize your changes." %>
</P>

<FORM NAME="form1" METHOD="post" ACTION="doc_edit.asp">
...
<TR>
	...
	<% Do Until rsBook.EOF %>
	<OPTION VALUE="<%= rsBook.Fields("BookID").Value %>"<% If steRecordEncValue(rsDoc, "BookID") = CStr(rsBook.Fields("BookID").Value) Then Response.Write " SELECTED" %>> <%= rsBook.Fields("Title").Value %>
	<%	rsBook.MoveNext
	   Loop
	rsBook.Close
	Set rsBook = Nothing %>
	...
	<TD>
	<% Call treTreeSelect("ParentDocID", "tblDoc", "DocID", "ParentDocID", "Title", "DocID <> " & nDocID, _
		steRecordEncValue(rsDoc, "ParentDocID")) %>
	</TD>
</TR><TR>
	...
	<OPTION VALUE="0"> -- Choose --
	<% Do Until rsAuth.EOF %>
	<OPTION VALUE="<%= rsAuth.Fields("AuthorID").Value %>"<% If steRecordEncValue(rsDoc, "AuthorID") = CStr(rsAuth.Fields("AuthorID").Value) Then Response.Write " SELECTED" %>> <%= rsAuth.Fields("Title").Value & " " & rsAuth.Fields("FirstName").Value & " " & rsAuth.Fields("MiddleName").Value & " " & rsAuth.Fields("LastName").Value & " " & rsAuth.Fields("Surname").Value %>
	<%	rsAuth.MoveNext
	   Loop
	rsAuth.Close
	Set rsAuth = Nothing %>
	...
	<TD><TEXTAREA NAME="ShortDescription" COLS="48" ROWS="5" class="form" style="width:100%"><%= steRecordEncValue(rsDoc, "ShortDescription") %></TEXTAREA></TD>
</TR><TR>
	<TD CLASS="forml" VALIGN="top" nowrap><% steTxt "Body" %></TD><TD></TD>
	<TD><TEXTAREA NAME="Body" COLS="48" ROWS="16" class="form" style="width:100%"><%= steRecordEncValue(rsDoc, "body") %></TEXTAREA></TD>
</TR><TR>
	<TD CLASS="forml" VALIGN="top" nowrap><% steTxt "Section Name" %></TD><TD></TD>
	<TD><INPUT TYPE="TEXT" NAME="SectionName" VALUE="<%= steRecordEncValue(rsDoc, "SectionName") %>" SIZE="32" MAXLENGTH="100" class="form"></TD>
</TR><TR>
	<TD CLASS="forml" VALIGN="top" nowrap><% steTxt "Is Inline Content?" %></TD><TD></TD>
	<TD class="formd">
		<INPUT TYPE="radio" NAME="IsInlineContent" VALUE="1"<% If steRecordBoolValue(rsDoc, "IsInlineContent") Then Response.Write " CHECKED" %> class="formradio"> <% steTxt "Yes" %>
		<INPUT TYPE="radio" NAME="IsInlineContent" VALUE="0"<% If Not steRecordBoolValue(rsDoc, "IsInlineContent") Then Response.Write " CHECKED" %> class="formradio"> <% steTxt "No" %>
	</TD>
</TR><TR>
	<TD CLASS="forml" VALIGN="top" nowrap><% steTxt "Author Notes" %></TD><TD></TD>
	<TD><TEXTAREA NAME="AuthorNotes" COLS="42" ROWS="10" WRAP="Virtual" class="form" style="width:100%"><%= steRecordEncValue(rsDoc, "AuthorNotes") %></TEXTAREA></TD>
</TR><TR>
	<TD COLSPAN=3 ALIGN="right"><br><INPUT TYPE="submit" NAME="_submit" VALUE=" <% steTxt "Update Document" %> " class="form"></TD>
</TR>
</TABLE>
</FORM>
...
<H3><% steTxt "Document Updated" %></H3>

<P>
<% steTxt "The document was successfully updated." %>&nbsp;
